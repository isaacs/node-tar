...
const timer = (start, msg) => {
  ...
  process.emit('timerlog', '  HEADER ' + t + space + msg)
}
...
class Header {
  constructor (data) {
    ...
    timer(this.timer, 'before isBuffer decode/encode')
  ...
  [DECODE] (block) {
    ...
    timer(this.timer, '-- before detect fieldset')
    const fieldset = this[FIELDSET] = detectFieldset(block)
    timer(this.timer, '-- after detect fieldset')
    this.nullBlock = true

    timer(this.timer, '-- decode before field setting')
    for (let i in fieldset) {
      this[i] = fieldset[i].read(block)
      if (this[i])
        this.nullBlock = false
    }
    timer(this.timer, '-- decode after field setting')
    ...
    timer(this.timer, '-- decode after checksum validation')
