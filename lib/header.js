    ...
    switch (field) {
      ...
      case "name":
        ...
        var nameLen = Buffer.byteLength(obj.name)
          , nameFSize = fieldSize[fields.name]
          , prefFSize = fieldSize[fields.prefix]

        // names between 100 and 255 should use the prefix field.
        // longer than 255
        if (nameLen > nameFSize &&
            nameLen <= nameFSize + prefFSize) {
          // need to find a slash somewhere in the middle so that
          // name and prefix both fit in their respective fields
          var searchStart = nameLen - 1 - nameFSize
            , searchEnd = prefFSize
            , found = false
