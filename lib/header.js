...
function encode (obj) {
  ...
  for (var f = 0; fields[f] !== null; f ++) {
    ...
    switch (field) {
      ...
      case "prefix":
        // special, this is an extension of the "name" field.
        break

      case "name":
        // uses the "prefix" field if > 100 bytes, but <= 255
        var nameLen = Buffer.byteLength(obj.name)

        // names between 100 and 255 should use the prefix field.
        // longer than 255
        if (nameLen > 100 && nameLen <= 255) {
          // need to find a slash somewhere in the middle so that
          // name <= 100, and prefix <= 155
          var searchStart = nameLen - 100
            , searchEnd = 155
            , found = false
            , nameBuf = new Buffer(obj.name)

          for (var s = searchStart; s <= searchEnd && false !== found; s ++) {
            if (nameBuf[s] === slash || nameBuf[s] === bslash) {
              found = s
            }
          }

          if (found !== false) {
            prefix = nameBuf.slice(0, found).toString("utf8")
            name = nameBuf.slice(found + 1).toString("utf8")
            console.error("found a slash at %d", found)
            console.error("prefix=%j", prefix)
            console.error("name=%j", name)

            ret = writeText(block, off, end, name)
        ...
        // names less than 100 chars don't need a prefix
        // and names longer than 255 need an extended header and will fail
...
function decode (block) {
  ...
  // if we got a prefix, then prepend it to the name.
