...
inherits(Reader, fstream.Reader)
...
function Reader () {
  ...
  me._stream.on("end", function () {
    if (!me._ended) me.emit("error", new Error("unexpected eof"))
    me.emit("end")
  })
...
Reader.prototype.resume = function () {
  // console.error(" reader resume!")
  if (this._stream) {
    // console.error("  stream resume")
    this._stream.resume()
  } else {
...
Reader.prototype.pause = function () {
  // console.error(" reader pause!")
  if (this._stream) {
    // console.error("  stream pause")
    this._stream.pause()
  } else {
    // console.error("  no stream")
  }
