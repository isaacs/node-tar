...
module.exports = class Parser extends EE {
  ...
  consumeHeader (chunk) {
    ...
    const entry = this[ENTRY] = new Entry(header, this[EX], this[GEX])
  ...
  [NEXTENTRY] () {
    while (this[QUEUE].length) {
      ...
      this.emit('entry', entry)
      if (!entry.endEmitted)
        return entry.on('end', _ => this[NEXTENTRY])
    }
  }
  ...
  consumeEntryBody (chunk) {
    return this.consumeBodyChunk(chunk, data => this[ENTRY].write(data))
  }

  // used by consumeMetaBody and consumeEntryBody
  // updates this[ENTRY] and this[STATE]
