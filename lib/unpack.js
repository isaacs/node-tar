...
class UnpackSync extends Unpack {
  ...
  [FILE] (entry) {
    ...
    let stream
    try {
      stream = new fsm.WriteStreamSync(entry.absolute, {
        mode: mode,
        autoClose: false
      })
    } catch (er) {
      try { fs.closeSync(stream.fd) } catch (er) {}
      return this[ONERROR](er, entry)
    }
    stream.on('error', er => this[ONERROR](er, entry))
    stream.on('finish', _ => {
      try {
        if (entry.mtime && !this.noMtime)
          fs.futimesSync(stream.fd, entry.atime || new Date(), entry.mtime)

        if (this[DOCHOWN](entry))
          fs.fchownSync(stream.fd, this[UID](entry), this[GID](entry))
      } catch (er) {
        this[ONERROR](er, entry)
      } finally {
        try { fs.closeSync(stream.fd) } catch (er) {}
